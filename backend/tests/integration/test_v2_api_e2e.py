...
def _issue_admin_token(org_id: uuid.UUID) -> str:
...
@pytest.fixture
def _auth_headers(test_org: uuid.UUID) -> dict[str, str]:
    return {"Authorization": f"Bearer {_issue_admin_token(test_org)}"}
...
async def _cleanup_connector_ids():
    ...
    async with SessionLocal() as db:
        for cid in created:
            await db.execute(text("DELETE FROM connectors WHERE id = :id"), {"id": cid})
        await db.commit()
...
async def test_full_pipeline_register_sync_list(
    ...
    async with app_client as client:
        ...
        resp = await client.get("/v1/connectors/available", headers=_auth_headers)
        ...
        resp = await client.post(f"/v1/connectors/{connector_id}/test", headers=_auth_headers)
        assert resp.status_code == 200
        body = resp.json()
        assert body["connected"] is True

        # 4. Sync — should discover the 10 mock assets
        resp = await client.post(f"/v1/connectors/{connector_id}/sync", headers=_auth_headers)
        ...
        resp = await client.get(f"/v1/assets?connector_id={connector_id}", headers=_auth_headers)
        assert resp.status_code == 200
        assets = resp.json()
        ...
        sample = assets[0]
        resp = await client.get(f"/v1/assets/{sample['id']}", headers=_auth_headers)
        assert resp.status_code == 200
        detail = resp.json()
        ...
        resp = await client.get(f"/v1/assets/{sample['id']}/history", headers=_auth_headers)
        assert resp.status_code == 200
        history = resp.json()
        ...
        resp = await client.get("/v1/discovery/status", headers=_auth_headers)
        assert resp.status_code == 200
        status = resp.json()
        ...
        resp = await client.get("/v1/dashboard/summary", headers=_auth_headers)
        assert resp.status_code == 200
        summary = resp.json()
...
async def test_search_finds_assets_by_name(
    ...
    async with app_client as client:
        ...
        await client.post(f"/v1/connectors/{connector_id}/sync", headers=_auth_headers)

        # Mock fixture includes "GPT-4o" in its name
        resp = await client.get("/v1/assets/search?q=GPT", headers=_auth_headers)
        assert resp.status_code == 200
        hits = resp.json()
