...
| **Runtime agent** — inline proxy, 3-stage pipeline | [`runtime-agent/`](../runtime-agent) | ✅ ships | 64 Go tests. Coverage: policy 75.3%, proxy 63.6%, controlplane 82.8%; **`management` 0%, `cmd/agent` 0%**. No benchmark harness at all. Stage 2 is hardcoded fail-open regardless of policy (GAP-004); a cold start with no policy is unconditionally fail-open (GAP-003). |
...
| **SDK fail-closed** — Python + Node | [`sdks/`](../sdks) | ✅ ships | **Zero tests. No CI job.** The branch deciding whether unprotected LLM traffic ships is unverified — the most severe row in this document. Pulled to the front of Phase 1 (GAP-005). |
