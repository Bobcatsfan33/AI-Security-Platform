...
**Was:** `backend/pyproject.toml` pinned loose lower bounds, so a local venv and
a fresh CI install resolved **different major versions**. Found the hard way in
Phase 0: locally `fastapi==0.136.1`, CI resolved `0.139.2`, and the tier tests
passed locally and failed on CI. FastAPI 0.139 had made `include_router` lazy —
routing was identical, only introspection changed. The app was fine and the
*tests* were wrong, which is the expensive kind: CI was right and there was no
local way to discover it.

Guardrail 4 says "full test suite green before every commit". That guarantee is
worth very little when the two suites run against different libraries.

**Closed by:** `backend/requirements.lock` — a universal, fully-hashed
resolution (98 packages, 2127 hashes) generated by `scripts/lock.sh`
(`uv pip compile --universal --generate-hashes`). CI installs from it with
`pip install --require-hashes`, and so does a developer: **the same file, the
same bytes**. Universal so one lock serves every interpreter in
`requires-python` (>=3.11) and both linux CI and macOS dev, rather than a
per-platform lock that is only true where it was generated. The lock pins
`fastapi==0.139.2` — the version CI was already resolving — so local now matches
CI rather than the reverse.

Two mechanisms keep it honest, because a lock has two failure modes:

* **drift** — a `Lock is in sync with pyproject` CI step recompiles and diffs.
  A lock that has drifted is worse than none: it reports a resolution nobody
  asked for while looking authoritative.
* **staleness** — pinning by hash means a dependency with a CVE stays pinned.
  `.github/dependabot.yml` covers pip, npm (frontend + Node SDK), gomod and the
  GitHub Actions themselves; `pip-audit` in CI is the backstop.

**Supply chain, not dev hygiene.** `--require-hashes` means every artifact must
match a hash committed to this repo, so a swapped or compromised upstream fails
the build instead of executing in it. It is also what makes the existing
CycloneDX SBOM and SLSA provenance describe something *reproducible* — an SBOM
generated from a floating resolution documents one machine's afternoon.
